///  @file Dijkstras.cpp
///  @brief The functions to make a path using Dijkstras

#include "dijkstras.h"

struct less_that_Gdistance
{
	inline bool operator() (node* node1, node* node2)
	{
		return(node1->Getm_gDist() < node2->Getm_gDist());
	}
};
...
std::vector<node*> Djikstras(std::vector<std::vector<node>> *_grid, node* _start)
{
	...
	while (goal == false && open.size() > 0)
	{
		...
		{
			SearchNodesD(open.front(), &open, _grid);
			open.erase(open.begin());
		}
		std::sort(open.begin(), open.end(), less_that_Gdistance());
	}
	return path;
}
...
void SearchNodesD(node *_front, std::vector<node*> *_open, std::vector<std::vector<node>> *_grid)
{
	if (_front->Getm_yHeu() != 0)
	{
		...
		if (above->Getm_isOpen() != true && above->Getm_isObject() != true)
		{
			...
			above->Calculatem_gDist(above->Getm_parNode(), 1.0f);
	...
	if (_front->Getm_yHeu() != 0 && _front->Getm_xHeu() != 29)
	{
		...
		if (diagUpRight->Getm_isOpen() != true && diagUpRight->Getm_isObject() != true)
		{
			...
			diagUpRight->Calculatem_gDist(diagUpRight->Getm_parNode(), 1.4f);
	...
	if (_front->Getm_xHeu() != 29)
	{
		...
		if (right->Getm_isOpen() != true && right->Getm_isObject() != true)
		{
			...
			right->Calculatem_gDist(right->Getm_parNode(), 1.0f);
	...
	if (_front->Getm_yHeu() != 30 && _front->Getm_xHeu() != 29)
	{
		...
		if (diagDownRight->Getm_isOpen() != true && diagDownRight->Getm_isObject() != true)
		{
			...
			diagDownRight->Calculatem_gDist(diagDownRight->Getm_parNode(), 1.4f);
	...
	if (_front->Getm_yHeu() != 30)
	{
		...
		if (down->Getm_isOpen() != true && down->Getm_isObject() != true)
		{
			...
			down->Calculatem_gDist(down->Getm_parNode(), 1.0f);
	...
	if (_front->Getm_yHeu() != 30 && _front->Getm_xHeu() != 0)
	{
		...
		if (diagDownLeft->Getm_isOpen() != true && diagDownLeft->Getm_isObject() != true)
		{
			...
			diagDownLeft->Calculatem_gDist(diagDownLeft->Getm_parNode(), 1.4f);
	...
	if (_front->Getm_xHeu() != 0)
	{
		...
		if (left->Getm_isOpen() != true && left->Getm_isObject() != true)
		{
			...
			left->Calculatem_gDist(left->Getm_parNode(), 1.0f);
	...
	if (_front->Getm_yHeu() != 0 && _front->Getm_xHeu() != 0)
	{
		...
		if (diagUpLeft->Getm_isOpen() != true && diagUpLeft->Getm_isObject() != true)
		{
			...
			diagUpLeft->Calculatem_gDist(diagUpLeft->Getm_parNode(), 1.4f);
